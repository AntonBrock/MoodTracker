//
//  PausableAnimation.swift
//  MoodTrackerSwiftUI
//
//  Created by ANTON DOBRYNIN on 19.09.2023.
//
...
struct PauseableLinearAnimationModifier: AnimatableModifier {
    var isActive: Bool
    ...
    func body(content: Content) -> some View {
        content
            .animation(.linear(duration: isActive ? duration : 0))
    }
}

extension View {
    func pauseableAnimation(isActive: Bool, duration: TimeInterval) -> some View {
        modifier(PauseableLinearAnimationModifier(isActive: isActive, duration: duration))
    }
}
